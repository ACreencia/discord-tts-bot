...
| `!change <voice_id>` | None | changes your tts voice** to <voice_id>   | `!change GBv7mTt0atIp3Br8iCZE`     |
| `!usage`   | `!use`, `!idk`, `!?` | displays a help message with all the available commands  | `!usage` |

** You can only use premade voice id's at the current moment, and you can find a list [here](https://elevenlabs.io/docs/voices/premade-voices). If you want to use custom voices, it only works for custom voices tied to the elevenlabs api key account owner
